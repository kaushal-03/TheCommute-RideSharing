...
app.post('/find.html',(req,res)=>{
    ...
    conn.query(sql,(error,results)=>{
        ...
        results.forEach(element => {
            ...
            //i= i+1
            //res.render("index",{source:element.source,destination:element.destination,date:element.journeydate})
            sources.push(element.source);
            destinations.push(element.destination);
            dates.push(element.journeydate);
...
app.get('/out',(req,res)=>{
  res.render("index",{sourcees:sources,destinatiions:destinations,datees:dates})
  sources=[];
  destinations=[];
  dates=[];
})
